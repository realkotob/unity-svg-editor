...
namespace SvgEditor.UI.Canvas
{
    internal sealed class ElementGestureHandler
    {
        ...
        private readonly DragController _elementDragController;
        ...
        public void BeginMove(GestureState state, int pointerId, DragBeginRequest request)
        {
        ...
        {
            Result<Unit> result = _elementDragController.CommitDrag(new CommitDragRequest(_host, dragMode, canvasDelta));
            if (result.IsSuccess)
            {
                if (_host.HasDefinitionProxySelection)
                {
