...
namespace SvgEditor.UI.Canvas
{
    internal sealed class CanvasSelectionVisualService
    {
        ...
        private bool TrySetDraggingSelectionVisual(PreviewSnapshot previewSnapshot)
        {
            ...
            if (_pointerDragController.DragMode == DragMode.RotateElement)
            {
                string elementKey = _host.SelectedElementKey;
                if (!string.IsNullOrWhiteSpace(elementKey) &&
                    _sceneProjector.TryResolveSelectedElementSceneRect(previewSnapshot, elementKey, out Rect rotatedSceneRect) &&
                    _sceneProjector.TrySceneRectToViewportRect(previewSnapshot, rotatedSceneRect, out Rect rotatedViewportRect))
                {
                    CanvasSelectionVisual rotationVisual = _sceneProjector.BuildSelectionVisual(
                        previewSnapshot,
                        new SelectionVisualRequest(
                            SelectionKind.Element,
                            rotatedViewportRect,
