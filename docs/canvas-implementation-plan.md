...
- 안정화된 항목
  - snapshot 계약 정리
    ...
    - `HitGeometry`
    - `BoundsQuality`
  - single selection / hover / selection overlay
  - move
  - 4개 corner handle 기반 scale
  - move / scale preview와 commit 경로 정리
  - top-level wrapper group 선택 방지
  - 문서 전환 후 stale bounding box 상태 초기화
- 이번 라운드에서 실제로 해결한 문제
  - drag 중에는 맞고 mouse up 후 bbox가 원래 위치로 돌아가던 문제
  - 이동 후 새 위치에서 hover / select / drag가 안 되던 문제
  - old hit area 와 new hit area 가 동시에 남던 문제
  - resize anchor 반영이 틀어지던 문제
- 구조적으로 바뀐 점
  - `CanvasElementDragController`는 drag-start snapshot 기준으로 delta를 계산한다.
  - `DocumentPreviewService`는 commit 후 `WorkingSourceText` 기준 fresh rebuild를 우선한다.
  - `PreviewSnapshotGeometryBuilder`는 local tessellation 후 world transform을 적용하는 방향으로 정리되었다.

현재 해석으로는 Plan 기준 `Phase 0 일부 + Phase 1 일부 + Phase 2 / 3 + Phase 4 일부`까지 온 상태다.
즉, 편집기 축은 어느 정도 고정됐지만 rotate / snapping / marquee / multi-select는 아직 시작 전이다.

## 14. Backlog

### 14.1 다음 우선순위

1. `preserveAspectRatio` 실제 지원
2. rotate
3. 8 handle resize + modifier
4. snapping
5. marquee / multi-select

### 14.2 단기 정리 작업

- `PreviewSnapshotGeometryBuilder`
  - rendered preview와 per-element geometry가 완전히 같은 source of truth를 쓰도록 추가 정리
  - synthetic fallback 경로가 필요한지 다시 점검
- projection / preview rect
  - preview image rect와 overlay projection rect 계약을 더 명시적으로 정리
- fixture / 회귀 확인
  - `viewBox` 있음 / 없음
  - negative coordinates
  - transformed parent `<g>`
  - group move / group resize
  - top-level wrapper group selection 방지
...
- bounding box / hover / select 회귀가 생기면 기능 추가보다 geometry / projection 계약 점검을 먼저 한다.
- temporary interaction workaround를 다시 늘리기보다 snapshot rebuild 경로를 먼저 의심한다.
- rotate를 넣기 전에 resize modifier 와 projection 계약을 먼저 정리한다.
